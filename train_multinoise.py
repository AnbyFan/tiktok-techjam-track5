#!/usr/bin/env python3
"""
Train a probe with multi-level feature noise augmentation.
Adds Gaussian noise at multiple sigma levels to simulate different
degradation strengths, improving robustness across the transform spectrum.

Usage:
    python train_multinoise.py --features features/sid_set features/comfy_aug --out probe_v5_multinoise
"""
...
def parse_args():
    ...
    p.add_argument("--out", default="probe_v5_multinoise")
...
def add_noise_level(X, sigma, rng):
    """Add noise at given sigma and re-normalize."""
    X_noisy = X + rng.normal(0, sigma, X.shape).astype(X.dtype)
...
def main():
    ...
    # Use 3 noise levels: 0.03, 0.06, 0.10 (covering the eval range)
    # Sample 25% of training data for each noise level
    noise_levels = [0.03, 0.06, 0.10]
    sample_frac = 0.25
    ...
    for sigma in noise_levels:
        ...
        X_noisy = add_noise_level(X_train[idx], sigma, rng)
        X_aug_parts.append(X_noisy)
        y_aug_parts.append(y_train[idx])
        print(f"[augment] sigma={sigma}: +{n_sample} samples")
    
    X_train_aug = np.vstack(X_aug_parts)
    y_train_aug = np.concatenate(y_aug_parts)
    print(f"[augment] total: {len(X_train)} -> {len(X_train_aug)} training samples")
    ...
    print(f"\n[test] acc={accuracy_score(y_test, (test_probs >= args.threshold).astype(int)):.4f}")
    ...
    config = {
        ...
        "model_type": "logistic_regression_multinoise",
