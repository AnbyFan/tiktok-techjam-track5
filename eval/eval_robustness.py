#!/usr/bin/env python3
"""
Robustness evaluation harness (Track 5).

Scores a labeled image set with the trained probe under clean conditions and
every transform/severity from the spec table, all at the SAME frozen
threshold loaded from probe_config.json:

    clean
    JPEG compression   quality = 90, 70, 50, 30
    Gaussian blur      sigma  = 0.5, 1.0, 2.0
    Resize             0.5x / 0.25x downscale, then upscale back
    Gaussian noise     sigma  = 0.02, 0.05, 0.10
    Color jitter       brightness/contrast/saturation +/-20%
    Center crop        80%

Inputs:
    --real-dir   folder of authentic images    (e.g. COCO val2017 subset)
    --ai-dir     folder of AI-generated images (e.g. DALL-E Advanced subset)
    --probe      folder with probe.joblib + probe_config.json (train_probe.py)

Outputs (written to --out):
    robustness_report.csv   per-transform metrics (machine-readable)
    robustness_report.md    compact table, paste-ready for README/Devpost
    predictions.csv         per image x transform scores for error analysis

Usage:
    python eval_robustness.py --real-dir data/val/real --ai-dir data/val/ai \
        --probe probe_v1 --out reports/val_run1
    # quick smoke test:
    python eval_robustness.py --real-dir data/val/real --ai-dir data/val/ai \
        --probe probe_v1 --max-per-class 50
"""
...
def parse_args():
    ...
    p.add_argument("--probe", required=True, help="dir with probe.joblib + probe_config.json")
    p.add_argument("--out", default="reports/run1")
    ...
    p.add_argument("--threshold", type=float, default=None,
                   help="override; default reads probe_config.json (recommended)")
...
def main():
    ...
    print(f"[probe] loaded {probe_dir/'probe.joblib'}  "
          f"(feature_dim={cfg['feature_dim']}, threshold={threshold})")
    ...
    # (path, label) with 0=real, 1=AI -- matches probe training mapping
        ...
        for batch in tqdm(batches, desc=name, unit="batch", leave=False):
            ...
            for path, label in batch:
                try:
                    ...
                    tensors.append(preprocess(apply_transform(img, kind, params, rng)))
    ...
    print(f"[saved] {out/'robustness_report.csv'}")
    print(f"[saved] {out/'robustness_report.md'}  <- paste-ready table")
    print(f"[saved] {out/'predictions.csv'}  <- mine this for FP/FN examples")
    print("[next] weak transforms -> targeted augmentation, then retrain "
          "and rerun this harness at the SAME threshold.")
