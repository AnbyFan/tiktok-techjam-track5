#!/usr/bin/env python3
"""
Robustness evaluation with MIRROR PADDING preprocessing.

Tests if mirror padding (instead of CLIP's default square resize)
improves robustness to aspect ratio distortion.

Usage:
    python eval_robustness_mirror_pad.py \
        --real-dir data/val/real \
        --ai-dir data/wildfake/Images/Diffusion_based/DALLE/DALLE/Advanced/DALLE3 \
        --config ensemble_config.json --max-per-class 500 --out reports/mirror_pad_eval
"""
...
def parse_args():
    ...
    p.add_argument("--out", default="reports/mirror_pad_eval")
...
def mirror_pad_square(img, target_size=224):
    ...
    then pad with mirror reflection to make it square."""
...
def main():
    ...
    print(f"[mode] MIRROR PADDING enabled")
        ...
        for batch in tqdm(batches, desc=name, unit="batch", leave=False):
            ...
            for path, label in batch:
                try:
                    ...
                    transformed = apply_transform(img, kind, params, rng)

                    # Apply mirror padding instead of standard preprocessing
                    img_padded = mirror_pad_square(transformed, target_size=224)

                    # Use scorer's normalization but our custom preprocessing
    ...
    print(f"[saved] {out/'robustness_report.md'}")
